...
def getbysearch(name: str):
    ...
    for item_id in Database:
        if Database[item_id]["counterparty"] == name:
            data[item_id] = Database[item_id]
        if Database[item_id]["instrument_id"] == name:
            data[item_id] = Database[item_id]
        if Database[item_id]["instrument_name"] == name:
            data[item_id] = Database[item_id]
        if Database[item_id]["trader"] == name:
            data[item_id] = Database[item_id]
...
def asset(name: str):
    data = {}
    for item_id in Database:
        if Database[item_id]["asset_class"] == name:
            data[item_id] = Database[item_id]
...
def price(minprice: float, maxprice: float):
    ...
    for item_id in Database:
        if (
            Database[item_id]["trade_details"]["price"] >= minprice
            and Database[item_id]["trade_details"]["price"] <= maxprice
        ):
            data[item_id] = Database[item_id]
...
def price(start: dt.datetime, end: dt.datetime):
    ...
    for item_id in Database:
        if (
            Database[item_id]["trade_date_time"] >= start
            and Database[item_id]["trade_date_time"] <= end
        ):
            data[item_id] = Database[item_id]
...
def tradetype(name: str):
    data = {}
    for item_id in Database:
        if Database[item_id]["trade_details"]["buysellindicator"] == name:
            data[item_id] = Database[item_id]
